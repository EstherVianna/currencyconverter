function Convert() {
  ...
  fetch(url + coins)
    ...
    .then(function(data) {
      ...
      if(value > 0 ){
      let euroReal = data.EURBRL;
      const currency = euroReal.ask;
      const elementoPai = document.querySelector(".container");
      const elementoFilho = document.createElement("input");
      const resposta = `${value} euros é igual a ${formatCurrency(value * currency)} reais`;
      ...
      paragraph.id = "date"
      const date =  new Date (euroReal.create_date);
